...
#;(define (fn-for-person p)
    (... (person-name p) (person-age p) (fn-for-lop (person-subs))))
...
(check-expect (under-20--person SIMON)
              (list (person-name KEVIN) ;found 1 node deep
                    (person-name HANNAH)))
(check-expect (under-20--person JAY)
              (list (person-name KEVIN) ;found 2 node deep
                    (person-name HANNAH)
                    (person-name JOHN)
                    (person-name SHANE)
                    (person-name BARBARA)
                    ...
                    (person-name KENNETH)
                    (person-name CHIN)))
(check-expect (under-20--person RAYN)
              (list (person-name RAYN) ;found at 1 then
...
(define (under-20--lop lop)
  (cond
    [(empty? lop) empty]
    [else
     (if (> MIN-AGE (person-age (first lop)))
