...
public class KdTree {
    ...
    public boolean contains(Point2D p) {
        if (p == null)  throw new IllegalArgumentException("cannot call contains with null ags");

        double[] pos = {p.x(), p.y()};
        Node search = contains(root, pos, 0);
        return search != null && search.p.equals(p);
    }
    ...
    private Node contains(Node node, double[] key, int level) {
        ...
        // cases must be aligned with how insert deals with equal cmp values
        if (cmp == 0 && Arrays.equals(nodePos, key)) {
            return node;
        }
        else if (cmp < 0)   search = contains(node.lb, key, level + 1);
        else                search = contains(node.rt, key, level + 1);
