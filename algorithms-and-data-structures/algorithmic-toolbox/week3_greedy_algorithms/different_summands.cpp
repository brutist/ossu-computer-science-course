...
vector<int>
optimal_summands_naive (int n)
{
  // Approach: Pick the smallest summand possible (+1) such that the previous
  // summand <
  //			than the current summand. If remaining n is smaller
  // than
  // the next 			summand, then last summand will be remaining n.
...
vector<int>
optimal_summands (int n)
{
  // Approach: Sum of n natural numbers n is equal to (k (k+1)) / 2.
  //			We can use this formula to speed up the algorithm from
  // O(n) 			O(k), in which k is the number of summands.
