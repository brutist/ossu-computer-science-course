...
int compute_min_refills(int dist, int tank, vector<int> & stops) {
    // Approach: Since the stops is a already sorted, we can think of it as numbers
    //          in a line. We can find the minimum refills needed by greedily
    //          covering the leftmost stops. If there are stops separated by 
    //          > tank distance, then it would not be possible to reach destination.
    ...
    for (unsigned int i = 1; i < N; i++) {
        ...
        bool last_stop_too_far_to_dest = (i == N - 1 && ((dist - curr_stop) > tank));
...
int compute_min_refills_naive(int dist, int tank, vector<int> & stops) {
    ...
    int n = stops.size();
    int min_refills = n + 1; // Initialize with a large number, as we're looking for the minimum
    stops.push_back(dist); // Add the final destination as a "stop"
    
    // There are 2^n subsets, and we will explore all subsets to find the minimum refills
    for (int i = 0; i < (1 << n); ++i) {
        ...
        // Check the final leg of the journey from the last stop to the destination
        if (dist - last_stop > tank) {
            possible = false;
        }
        
        // Update the minimum refills if this subset is valid and requires fewer refills
        if (possible) {
...
void stress_test_compute_min_refills() {
    ...
    while (true) {
        ...
        if (naive_answer != fast_answer){
            ...
            std::cout << "Car Fueling of n: " << n << "  answer: " 
                        << naive_answer << "  result: " << fast_answer << " tank distance: " 
                        << tank << "\n";
