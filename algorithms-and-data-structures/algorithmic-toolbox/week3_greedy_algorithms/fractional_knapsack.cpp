  ...
  for (int i = 0; i < N; i++)
    {
      ...
      // stop adding items if knapsack is full or ther are no remaining items
      // to add
...
get_optimal_value (int capacity, vector<int> weights, vector<int> values)
{
  ...
  //          items and pick the first available item and add it to the
  //          knapsack. This approach would simplify the O(n^2) naive approach
  //          to O(nlogn).
...
int
get_most_weighted_value (vector<int> &weights, vector<int> &values,
                         vector<int> &included)
{
  int N = weights.size ();
  int max_index = -1;
...
bool
equal_values (double value1, double value2, double precision)
{
      ...
      if (!equal_values (naive_answer, fast_answer, 4))
        {
          ...
          std::cout << "Fractional Knapsack of n: " << n
                    << "  answer: " << naive_answer
                    << "  result: " << fast_answer << " capacity: " << capacity
                    << "\n";
