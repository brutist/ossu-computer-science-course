...
double get_optimal_value_naive(int capacity, vector<int> weights, vector<int> values) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        // stop adding items if knapsack is full or ther are no remaining items to add
        if (total_capacity <= 0 || max < 0)  break;
...
int get_most_weighted_value(vector<int> & weights, vector<int> & values, vector<int> & included) {
    ...
    int max_index = -1;      
    for (int i = 0; i < N; i++) {
        if (weights[i] <= 0 || included[i] || i == max_index) continue;
    ...
    while (true) {
        ...
        if (!equal_values(naive_answer, fast_answer, 4)){
            ...
            std::cout << "Fractional Knapsack of n: " << n << "  answer: " 
                        << naive_answer << "  result: " << fast_answer << " capacity: " 
                        << capacity << "\n";
