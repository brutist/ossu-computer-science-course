...
vector<int> optimal_points(vector<Segment> &segments)
{
	// Approach: Select the minimum right endpoint. Remove all segments containing
	//			that endpoint. Keep choosing minimum right endpoint until done.
	...
	for (int i = 0; i < N; i++)
	{
		...
		//	segment has been considered (I think there is a much elegant way
		//	to avoid this last check but call me idiot later).
...
void stress_test_optimal_points()
{
	...
	while (true)
	{
		...
		if (naive_answer != fast_answer)
		{
			...
			std::cout << "Covering Segments of n: " << n << "  answer: "
