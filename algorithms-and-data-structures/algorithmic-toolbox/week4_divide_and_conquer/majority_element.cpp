...
int get_majority_element_improved_helper(vector<int> &a, int start, int end) {
    ...
    //          the whole array if it is at least the majority of one of the halves.
    ...
    for (int i = start; i <= end; i++) {
        if (left_majority == a[i])          left_majority_count++;
        else if (right_majority == a[i])    right_majority_count++;
    }
