...
#==============================================================================#
#   Usage: bash compare_cpp.sh <cpp_dir> <num_tests> [-v] [--tests] [--max] [--stress] [--compare]
...
#         Add --compare to enable comparison even with the --max flag.
#==============================================================================#

# Ensure at least two arguments are provided
if [ $# -lt 2 ]; then
    echo "Usage: $0 <cpp_dir> <num_tests> [-v] [--tests] [--max] [--stress] [--compare]"
    exit 1
fi
...
# Process additional optional arguments (-v, --tests, --max, --stress, and --compare)
for arg in "$@"; do
    case "$arg" in
        -v) verbose=true ;;
        --tests) create_tests=true ;;
        --max) use_max_flag=true ;;
        --stress) use_stress_flag=true ;;
        --compare) compare_flag=true ;;
    esac
done
...
if [ "$use_max_flag" = false ] || [ "$compare_flag" = true ]; then
    # Compile naive program if --max flag is not used or --compare is enabled
...
for i in $(seq -f "%04g" 0 $((num_tests-1))); do
    ...
    # Output comparison based on --compare flag
    if [ "$use_max_flag" = true ] && [ "$compare_flag" = false ]; then
        # Skip output comparison when --max is used and --compare is not enabled
