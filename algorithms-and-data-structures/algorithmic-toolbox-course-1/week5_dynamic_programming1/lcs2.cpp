...
class LCS {
  public:
    ...
    int lcs_naive_util(int n, int m) {
        ...
        return max(
            {lcs_naive_util(n - 1, m),       // remove a[last]
             lcs_naive_util(n, m - 1),       // remove ab[last]
             lcs_naive_util(n - 1, m - 1)}); // remove a[last] and b[last]
    }

    // wrapper for naive implementation of finding the longest common
    // subsequence
    int lcs2_naive() { return lcs_naive_util(a.size(), b.size()); }

    int find_max_subsequence(vector<vector<int>> &dp_array, int i, int j) {
        // Approach: The recurrence relation between the max subsequence M(a,b)
        // of two
        //          vectors Va and Vb is M(a,b) = max(M(a-1, b), M(a, b-1),
        //          M(a-1, b-1))
        if (i == 0 || j == 0) {
    ...
    void stress_test_lcs2() {
        ...
        while (true) {
            ...
            if (naive_answer != fast_answer) {
                std::cout << "Longest Subsequence NAIVE answer: "
                          << naive_answer << "\n";
