...
(define (main c)
  (big-bang c                          ; Cat
            ...
            (on-key    key-handler)))    ; Cat KeyEvent -> Cat
...
;; Cat KeyEvent -> Cat
;; reset the cat's position to the leftmost when the "space" key is pressed
;; examples/tests
(check-expect (key-handler 10 " ") 0)
(check-expect (key-handler 0 " ") 0)
(check-expect (key-handler 10 "a") 10)
(check-expect (key-handler 0 "a") 0)

;; stub
#;
(define (key-handler c ke) 0)

; <template from HTDW recipe>
(define (key-handler c ke)
  (cond [(key=? ke " ") 0]
        [else c]))
