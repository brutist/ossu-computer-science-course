...
(define BG ; 1nd round match-up:
  (make-bracket "Scandal" "Pheonix" false false)) ; - Scandal defeat Pheonix

(define BH ; 1nd round match-up:
...
(define BK ; 2nd round match-up:
  (make-bracket "Riot" "Nemesis" BE BF)) ; - Scandal defeat Capitals
...
(define (fn-for-lot lot)
  (cond
    [(empty? lot) (...)]
    [else (... (first lot) (fn-for-lot (rest lot)))]))
...
;; produce true if the ListOfTeams corresponds to the right order of teams beaten
;;    by the winner of the given bracket
...
;; examples/tests
(check-expect (lose-order false empty) true)
(check-expect (lose-order BL empty) true)
(check-expect (lose-order false T2) false)
(check-expect (lose-order BN (list "Riot")) true)
(check-expect (lose-order BN (list "Nemesis")) false)
(check-expect (lose-order BN (list "Riot")) true)
(check-expect (lose-order BN (list "Riot" "Capitals")) true)
(check-expect (lose-order BN (list "Riot" "Pheonix")) true)
(check-expect (lose-order BN (list "Riot" "Capitals" "Pheonix")) true)
(check-expect (lose-order BN (list "Riot" "Nemesis")) false)
(check-expect (lose-order BN (list "Riot" "Nemesis" "Schwa")) false)
(check-expect (lose-order BK (list "Nemesis" "Schwa")) true)
(check-expect (lose-order BK (list "Nemesis" "Ozone")) false)
(check-expect (lose-order BK (list "Nemesis" "Riot")) false)

#;(define (lose-order br lot)
    "false") ;stub
...
(define (lose-order br lot)
  (cond [(empty? lot) true]
        [(false? br) false]
        [else 
            (if (string=? (bracket-team-lost br) (first lot))   ;losing team found
                (lose-order (bracket-br-won br) (rest lot))     ;remove losing team in lot
                (lose-order (bracket-br-won br) lot))]))        ;keep losing team in lot
